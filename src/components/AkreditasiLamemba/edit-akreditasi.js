...
export default function EditAkreditasiDialog({
  open,
  onOpenChange,
  data,
  onSuccess,
  ...
  const [formData, setFormData] = useState({
    ...
    nilaiAkreditasi: data?.nilaiAkreditasi || "",
    keterangan: data?.keterangan || "",
  });

  const handleSubmit = (e) => {
    e.preventDefault();
    // Simulasi update - nanti bisa diganti dengan API call
    const updatedData = {
      ...data,
      ...formData,
      tanggalBerlaku: formData.tanggalBerlaku?.toISOString().split("T")[0],
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Data Akreditasi</DialogTitle>
          <DialogDescription>
            Ubah informasi akreditasi LAMEMBA
          </DialogDescription>
        </DialogHeader>
