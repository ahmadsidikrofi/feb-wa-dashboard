...
vec3 telkomBackground(vec2 uv) {
    float waveBase =
        sin(uv.x * 1.2 + iTime * 0.3) * 0.25 +
        sin(uv.x * 2.4 + iTime * 0.1) * 0.1;

    float d = uv.y - waveBase;

    vec3 c1 = TELKOM_DARK;
    vec3 c2 = TELKOM_RED;
    vec3 c3 = TELKOM_SOFT;

    float t1 = smoothstep(-1.0, 0.0, d);
    float t2 = smoothstep(0.0, 1.0, d);

    vec3 mix1 = mix(c1, c2, t1);
    vec3 mix2 = mix(c2, c3, t2);

    return mix(mix1, mix2, t2 * 0.7);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 b = telkomBackground(baseUv);
