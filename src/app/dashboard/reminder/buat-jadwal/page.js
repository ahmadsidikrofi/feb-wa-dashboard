    ...
    >
      <div className="min-w-0 flex-1 space-y-1">
        <div className="text-sm text-muted-foreground">
          Akan dikirim pada: {format(event.reminderTime, "HH:mm")}
        </div>
      ...
      <div className="flex shrink-0 items-center gap-2">
        <Badge
          ...
          variant={event.status === "sent" ? "default" : "secondary"}
        >
          {event.status}
        </Badge>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="min-w-40">
              <DropdownMenuItem disabled>
                <GalleryVerticalEnd className="size-4 mr-2" /> Detail
              </DropdownMenuItem>
              <DropdownMenuItem disabled>
                <PencilLine className="size-4 mr-2" /> Edit
              </DropdownMenuItem>
              <DropdownMenuItem
              ...
              >
                {isLoading ? (
                  ...
                  <X className="text-destructive size-4 mr-2" />
                )}{" "}
                Batalkan
              </DropdownMenuItem>
              <DeleteSchedule
...
const ReminderPage = () => {
  ...
  const createSchedule = async (values) => {
    ...
    try {
      ...
      const [reminderHours, reminderMinutes] = values.reminderTime
        .split(":")
        .map(Number);
      ...
      if (res.status === 201) {
        ...
        toast.success(`Jadwal pengingat sukses dibuat`, {
          description: `Yang akan dikirimkan pada ${format(
            combinedReminderDateTime,
            "HH:mm"
          )}`,
        });
        form.reset();
      ...
      toast.error(
        "Gagal membuat pengingat, jangan sungkan untuk mencobanya lagi"
      );
    } finally {
      setIsLoading(false);
      setOpen(false);
  ...
  const handleCancelSchedule = async (scheduleId) => {
    ...
    try {
      const res = await axios.put(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/schedules/${scheduleId}/cancel`,
        {
          status: "cancelled",
        }
      );
          ...
          <DialogContent className="max-w-lg">
            ...
            <Form {...form}>
              <form
                ...
                className="grid gap-4 py-2 max-h-[60vh] overflow-y-auto pr-1 space-y-2"
              >
                <FormField
                  control={form.control}
                  name="eventTitle"
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder="Nama kegiatan yang akan dihadiri"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
                            ...
                            <FormControl>
                              ...
                              >
                                ...
                                {field.value ? (
                                  ...
                                  <span>Pilih tanggal reminder</span>
                                )}
                              </Button>
                            </FormControl>
                          </PopoverTrigger>
      ...
      <section className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1">
          <Card>
            <CardHeader>
              <CardTitle>
                Jadwal pengingat untuk {formatHumanDate(selectedDate)}
              </CardTitle>
            </CardHeader>
            <CardContent>
